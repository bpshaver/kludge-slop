...
- **Draw every correct-answer position from one up-front batch, before drafting any question — never per-question.** Your default habit is to place the correct answer first, and any pattern you pick by hand (rotation, "feels random") is predictable to a repeat user — so *draw the slots from an actual random source*. But the draw itself is a tool call, and its output lands in the same visible transcript as everything else: if you draw (or even re-read) a slot immediately before writing that question, the number shows up right next to the question it decides, and it'll register in the user's field of view as they read straight through — an accidental spoiler, not a deliberate cheat. Avoid that by decoupling the draw from any specific question in time:
  - At the very start of the session, before any beat or question is drafted, run **one** shell command that draws a generous batch of raw random integers — e.g. `for i in $(seq 1 30); do echo $((RANDOM % 1000)); done` — enough to cover every quiz question you might ask this session. This is the only random draw and the only read of it for the rest of the session.
  - Record the full batch in the tracker (not shown to the user) and take the numbers in order, one per quiz question, crossing each off as used.
  - For each question, convert its next raw number to a slot with plain arithmetic in your own reasoning — `(raw_number % option_count) + 1` — so it works regardless of that question's option count. This conversion is just you thinking, not a tool call, so nothing new gets printed.
  - If you exhaust the batch, draw one fresh batch the same way (start of a beat, not mid-question) rather than falling back to a per-question draw.
  Place the correct answer in the resulting slot and fill the rest with distractors. Distractors must be plausible, not obvious throwaways. (Quizzes only; grills have no correct answer.)
...
- Quiz options: one correct, the rest plausible distractors. Place the correct one in the slot from the pre-drawn batch per the Golden rules (next unused raw number, `% option_count + 1`) — never a fresh draw or file read for this question — and keep every option's `description` even in length/detail so the text doesn't give it away.
